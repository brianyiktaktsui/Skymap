...
cmd_1='bowtie2  -x {ref} {srrRun}_bowtie --threads 2 > {srrRun}_count 2>bowtie_log.txt'.format(ref=genomeDir,srrRun=srrRun)
#take in and sort it 
if TEST:
    cmd_2_fmt='{sra_dump_dir} --stdout {sra_file_dir} | head -n 100000 |cutadapt -a {adapter_sequence_1} - > {srrRun}_bowtie'
else:
    cmd_2_fmt='{sra_dump_dir} --stdout {sra_file_dir} |cutadapt -a {adapter_sequence_1} - > {srrRun}_bowtie'
