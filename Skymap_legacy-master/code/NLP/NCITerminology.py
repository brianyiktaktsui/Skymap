...
class NCITerminology:
    ...
    def _readOnto(self):
        ...
        tmpGraph=nx.from_pandas_dataframe(df=self.allEdgeDf,source='src',target='dest',edge_attr=['rel'],create_using=nx.DiGraph())
    ...
    def setCellOntoEdgeSubset(self,rels):
        ...
        self.cellOntoSubset=nx.from_pandas_dataframe(df=tmpDf,source='src',target='dest',edge_attr=['rel'],create_using=nx.DiGraph())
