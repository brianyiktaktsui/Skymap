...
import FMAOntologyParam as param
param=reload(param)
import networkx as nx
...
class FMAOntology:
    ...
    def _readOnto(self):
        ...
        self.revG=self.cellOntoSubset.reverse()
    """
    input: terms
    ...
    def egoGraph(self,node,radius=2,eng=False):

        children=nx.ego_graph(self.revG,node,radius=radius).nodes()
        return [self.cleanedIdS[c] for c in children]  if eng else children
